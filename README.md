...
```
node scripts/build.mjs          # 七步：打包运行时 → arg-data.js → arg-vfs.js → 页面瘦身 → 三指纹 → 索引 → 防旁路校验
node scripts/validate.mjs       # 完整性 + 防旁路硬门槛（断锁后核心线索不可达、结局门槛可满足性）
node scripts/golden-path.mjs    # 13 个结局可达性模拟（须解题 vs 随手可达）
node --test "tests/**/*.test.mjs"   # 54 项：算法/门控/索引/运行时冒烟/结局/VFS/窗口
npm run gen:all                 # 一键重建硬核谜题/小游戏/ARG 内容 + 构建（幂等）
```

整站 `files/names` 集中在共享 `arg-data.js`、桌面资源管理器的文件树在共享 `arg-vfs.js`（各页不再内嵌大表，页面体积约降 60%：15.7MB→4.1MB）。桌面/文件柜页自动加载**潮声OS 窗口系统 + 资源管理器**（Win98/XP/Classic 三皮肤可切换）。`arg-runtime.js`、`arg-search-index.js`、`arg-data.js`、`arg-vfs.js` 均为构建产物：改运行时请改 `src/runtime/*.mjs` 后重新构建，勿直接编辑产物。CI 见 `.github/workflows/ci.yml`。架构与「如何安全地加谜题」见 `docs/ENGINEERING.md`。
