...
> 一个大型原创 ARG 风格桌面解谜游戏：**102 个页面 · 4 道锁 · 10 层密码 · 9 个结局（含打破第四面墙的 ARG 结局）**。
...
- 在「桌面 / 加密聊天 / 千禧搜索 / 蓝湾潮声论坛 / 档案馆 / 维基 / 保险箱 / 机房 / 灯塔顶层」之间搜集线索。
- 每打开一个页面记一条线索（存 **localStorage**，关标签页不丢；右下角「线索 X/? ⟳」可看进度、点 ⟳ 重置）。
- **结案＝指认嫌疑人**（聊天里的记者"明月如霜"）。游戏里有**伪造证据**——看了它们只会解锁坏结局。
- **10 层密码链**：凯撒 → Base64 → 摩尔斯 → 组合锁 → 十六进制隐写 → 维吉尼亚 → 元素周期表 → 二进制 → 倒读密文 → **地址栏 hash**。
- **ARG 层要动真的浏览器**：在地址栏加 `#firstlight`、查真的元素周期表、用转换器解二进制、把整页倒过来读、拿真实经纬度去地图上找那片海。
...
├── *.html                # 其余 101 个页面（4 道锁 / 10 层密码题 / ARG 谜题页 / 9 个结局页）
├── arg-runtime.js        # 线索记录(localStorage) / 路由 / 打字机 / 白噪 / hash 谜题运行时
