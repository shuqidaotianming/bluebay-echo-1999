...
### Added
- **潮声OS 会话生命周期** `osboot.mjs`：仿真 Windows 的**开机 POST（BIOS 自检）→ 带密码登录（沈知微）→ 关机 / 重新启动 / 休眠 / 锁屏** 全屏界面；`osshell` 在桌面与文件柜页先过开机+登录再进桌面；任务栏加时钟 + ⏻ 电源菜单。
- **文件柜/回收站改用资源管理器呈现**：`osexplorer` 支持"专属文件夹"，`osshell` 在 `.folder` 类页面隐藏旧模板界面、直接打开该柜/回收站的文件视图；VFS 新增「🗑️ 回收站」分组；桌面常驻「💻 我的电脑 / 🗑️ 回收站」图标。
- 防泄露加固（本轮）：`apply-locks` 从蓝图自动推导每把锁私有子树、补 `requiresClue=lock:`（仅"破解"才解锁，路过登录页不算）；`hasClue` 认 `lock:`；校验器新增"锁门控页必须带 requiresClue""搜索白名单不得含锁门控/结局页"两道硬门；Explorer 对锁定项隐名。移除 `doc_weather_log` 的可搜索泄露。
- 窗口内文档阅读器 + 潮声信箱邮件客户端 + 控制面板（皮肤/无障碍/存档）。
